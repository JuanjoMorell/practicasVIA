...
for n, (key,frame) in enumerate(stream):
    ...
    if len(poses) == 2:
        ...
        posicionFinal = []
        posicionFinal.append([(pos[0][0][0] + pos[1][0][0]) / 2,(pos[0][0][1] + pos[1][0][1]) / 2 ])
        posicionFinal.append([(pos[0][1][0] + pos[1][1][0]) / 2,(pos[0][1][1] + pos[1][1][1]) / 2 ])
        posicionFinal.append([(pos[0][2][0] + pos[1][2][0]) / 2,(pos[0][2][1] + pos[1][2][1]) / 2 ])
        posicionFinal.append([(pos[0][3][0] + pos[1][3][0]) / 2,(pos[0][3][1] + pos[1][3][1]) / 2 ])
        posicionFinal = np.array(posicionFinal)
        print(posicionFinal)

        H = cv.getPerspectiveTransform(src.astype(np.float32), posicionFinal.astype(np.float32))
        # la aplicamos encima de la imagen de cámara
        cv.warpPerspective(imvirt,H,size,frame,0,cv.BORDER_TRANSPARENT)

        # Mostrar una imagen en medio
    else:
        for M in poses:
